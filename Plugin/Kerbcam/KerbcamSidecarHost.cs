...
// GameObject created lazily by the first flight-scene KerbcamCore.Awake
...
namespace Kerbcam
{
    internal sealed class KerbcamSidecarHost : MonoBehaviour
    {
        ...
        public static KerbcamSidecarHost Instance { get; private set; }
        ...
        /// entry. Called from every KerbcamCore.Awake; only the first call
        /// of the session spawns the process.
        /// </summary>
        public static void NotifyFlightEntered(KerbcamSettings settings, string ringDir)
        {
            if (Instance == null)
            {
                var go = new GameObject("KerbcamSidecarHost");
                UnityEngine.Object.DontDestroyOnLoad(go);
                Instance = go.AddComponent<KerbcamSidecarHost>();
                Instance.Init(settings, ringDir);
            }
            Instance.OnFlightEntered(settings);
        }

        private void Init(KerbcamSettings settings, string ringDir)
        {
            _settings = settings;
            _ringDir = ringDir;
            ...
            WriteHeartbeat();
            Debug.Log("[Kerbcam] sidecar host created (persists for the KSP session)");
        }

        private void OnFlightEntered(KerbcamSettings freshSettings)
        {
        ...
        /* Process-level flags are baked into the running sidecar's CLI; a
           ...
           every flight entry via KerbcamCore. */
        private void WarnIfProcessSettingsChanged(KerbcamSettings fresh)
        {
            ...
            {
                Debug.LogWarning(
                    "[Kerbcam] process-level settings (bind/port, Width/Height, BitrateBps, "
        ...
        private void Update()
        {
            ...
            if (_sidecarExited)
            {
                ...
                if (verdict.GaveUp)
                {
                    Debug.LogError($"[Kerbcam] sidecar crashed {verdict.MaxAttempts} times; giving up until the next flight scene");
                }
                else if (verdict.Attempt > 0)
                {
                    Debug.LogWarning(
                        $"[Kerbcam] sidecar exited unexpectedly (code {_lastExitCode}); "
        ...
        private void WriteHeartbeat()
        {
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogWarning($"[Kerbcam] heartbeat write failed (orphan protection degraded): {ex.Message}");
            }
        }
        ...
        // from another shell, which is how kerbcam was originally built and
        ...
        private void TryStartSidecar()
        {
            try
            {
                ...
                _sidecar.Exited += (sender, e) =>
                {
                    ...
                    if (_expectedExit)
                    {
                        Debug.Log($"[Kerbcam] sidecar stopped (game exit, code {code})");
                        return;
                    }
                    _lastExitCode = code;
                    _sidecarExited = true;
                ...
                _lifecycle.OnSpawned();
                Debug.Log($"[Kerbcam] sidecar started pid={_sidecar.Id} from {binPath} (runs until game exit)");
            }
            catch (Exception ex)
            {
