// KSP Difficulty Settings integration for kerbcast's runtime-toggleable
...
//
// Default values are seeded from `settings.cfg` (KerbcastSettings) the
// first time a save loads with kerbcast present. After that the value
// written into the save by the operator (via Pause → Difficulty Settings
// → Kerbcast) wins on every subsequent load. Sync between settings.cfg
...
namespace Kerbcast
{
    public class KerbcastGameParameters : GameParameters.CustomParameterNode
    {
        public override string Title => "Kerbcast";
        public override string DisplaySection => "Kerbcast";
        public override string Section => "Kerbcast";
        ...
        // kerbcast's per-Hullcam encode pipeline. Pop-up GUI overlay
        ...
        [GameParameters.CustomParameterUI(
            ...
            toolTip =
                "Disable KSP's main flight cameras to free GPU/CPU for kerbcast streams. " +
        ...
        public KerbcastGameParameters()
        {
            ...
            // the field stored.
            ThrottleMainScreen = KerbcastSettings.SeedThrottleMainScreen;
        }
    }
}
