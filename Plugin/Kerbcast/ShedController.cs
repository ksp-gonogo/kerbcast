...
//
// Background: KerbcastCore samples a rolling fps average and asks this controller
// each LateUpdate whether to raise or lower the shed level. Each level maps (in
// KerbcastCamera.ShedTable) to a render-resolution multiplier + a set of FX
...
namespace Kerbcast
{
    ...
    public sealed class ShedController
    {
        ...
        //
        // DEFAULTS (quality-shed semantics): the first shed only triggers below
        // 15 fps — kerbcast tolerates a slow-but-playable game and only degrades
        // *quality* when genuinely struggling. The STAGGER controller passes its
        // own, earlier thresholds (≈30 fps target) via the constructor, because
        // staggering is lossless and worth engaging proactively to hold fps.
        private static readonly float[] DefaultShedBelow    = { 15f, 12f, 9f, 6f, 3f };
        private static readonly float[] DefaultRestoreAbove = { 20f, 17f, 14f, 11f, 8f };

        // Instance thresholds (default or constructor-supplied) + the cost-share
        // escalation gate. _shedBelow.Length sets the effective max level.
        private readonly float[] _shedBelow;
        private readonly float[] _restoreAbove;
        // Minimum fraction of frame time kerbcast must occupy before we escalate.
        // Guards the absolute-fps-target failure mode: if the game is slow for
        // reasons that AREN'T kerbcast (heavy vessel, physics), staggering can't
        // raise fps, so escalating would just starve every feed chasing an fps
        // the game can't hit. 0 = no gate (quality-shed default).
        private readonly double _minEscalateShare;
        ...
        public ShedController(
            ...
            float[] shedBelow = null,
            float[] restoreAbove = null,
            double minEscalateShare = 0.0)
        {
            _shedBelow = shedBelow ?? DefaultShedBelow;
            _restoreAbove = restoreAbove ?? DefaultRestoreAbove;
            if (_restoreAbove.Length != _shedBelow.Length)
                throw new ArgumentException("shedBelow and restoreAbove must be the same length");
            _minEscalateShare = minEscalateShare;
            if (maxLevel < 0) maxLevel = 0;
            // Can't shed past the number of thresholds we have.
            if (maxLevel > _shedBelow.Length) maxLevel = _shedBelow.Length;
            _maxLevel = maxLevel;
        ...
        /// </summary>
        /// <param name="kerbcastShare">
        /// Fraction of the current frame time attributable to kerbcast's
        /// main-thread work (0..1). Escalation is gated on this being &gt;=
        /// the controller's minEscalateShare: if kerbcast isn't a meaningful
        /// share of the frame, the slowness is the game's, not ours, and
        /// staggering harder can't help — so we hold instead of starving feeds.
        /// Defaults to 1.0 (gate always passes) for callers that don't supply it
        /// (the quality-shed instance, which uses minEscalateShare = 0 anyway).
        /// </param>
        public int Evaluate(float fpsAvg, double now, double kerbcastShare = 1.0)
        {
            double sinceChange = now - _lastChangeTime;

            // Escalate (shed) — we're overloaded at the current level AND kerbcast
            // is actually a meaningful share of frame time (else don't starve
            // feeds chasing an fps the game can't reach with kerbcast off).
            if (_level < _maxLevel && fpsAvg < _shedBelow[_level]
                && kerbcastShare >= _minEscalateShare)
            {
            ...
            if (_level > 0 && fpsAvg > _restoreAbove[_level - 1])
            {
