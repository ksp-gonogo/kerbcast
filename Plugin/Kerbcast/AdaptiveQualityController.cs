...
//   at its one-camera floor AND kerbcast is still over its ms budget (or the
//   game is below the MinKspFps physics floor). Each level maps, in
//   KerbcastCamera.ShedTable, to a render-resolution multiplier plus FX layers
...
//   count (temporal recovery comes first), kerbcast comfortably under its ms
...
namespace Kerbcast
{
    public sealed class AdaptiveQualityController
    {
        ...
        /// higher = more shed (see KerbcastCamera.ShedTable).</summary>
        ...
        /// </summary>
        /// <param name="kerbcastFrameMs">EMA of kerbcast's own main-thread cost
        ...
        public int Evaluate(double kerbcastFrameMs, int staggerBudget, int camCount, double gameFps, double now)
        {
            ...
            if (camCount <= 0 || kerbcastFrameMs <= 0.0)
            {
                ResetHeadroom();
                return _level;
            }
            ...
            if (staggerBudget <= 1 && (kerbcastFrameMs > over || belowFloor))
            {
                ResetHeadroom();
                if (_level < _maxLevel && now - _lastDemoteTime >= _demoteDwellSeconds)
                {
                    _level++;
                    _lastDemoteTime = now;
                    LastChangeReason = kerbcastFrameMs > over
