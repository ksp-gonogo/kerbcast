/* StaggerBudgetController regulates the per-frame capture budget (how many
   cameras render + read back this tick) to hold kerbcast's OWN main-thread cost
   ...
   target is a direct division. Targeting kerbcast's own cost (not game fps) is
   ...
   gates restores until fps clears floor + hysteresis, so kerbcast never relaxes
...
namespace Kerbcast
{
    public sealed class StaggerBudgetController
    {
        ...
        /// </summary>
        /// <param name="kerbcastFrameMs">Measured kerbcast main-thread cost this
        /// frame (the capture loop's wall-time, EMA-smoothed by the caller).</param>
        /// <param name="msPerCam">Cost per capturing camera (kerbcastFrameMs /
        ...
        public int Evaluate(double kerbcastFrameMs, double msPerCam, int camCount, double gameFps, double now)
        {
            ...
            if (kerbcastFrameMs <= 0.0 || msPerCam <= 0.0)
            {
                _budget = camCount;
                ResetSignalClocks();
                return _budget;
            ...
            bool overTrigger = kerbcastFrameMs > over || belowFloor;
            bool largeOverTrigger = kerbcastFrameMs > largeOver;
            bool underTrigger = kerbcastFrameMs < under && aboveFloor;
            ...
            /* RESTORE: one step per release dwell, only with room on BOTH
               counts (kerbcast under budget AND game fps comfortably above the
