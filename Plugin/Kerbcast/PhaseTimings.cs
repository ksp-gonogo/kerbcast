...
//
//   GcTracker — global GC-collection counters. Once per frame KerbcastCore
...
// no LINQ, no per-call allocation. Only the 1Hz status write (in KerbcastCore)
// builds strings, which the perf budget explicitly blesses.

namespace Kerbcast
{
    /// <summary>The distinct render phases timed inside KerbcastCamera.Refresh().
    ...
    /// KerbcastCamera; entirely Unity-free so it unit-tests standalone. All
